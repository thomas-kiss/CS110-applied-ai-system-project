...
This project extends **VibeMatch 1.0**, a content-based music recommender simulator built as part of CodePath Ai110 Spring 2026. [Repo](https://github.com/codepath/ai110-module3show-musicrecommendersimulation-starter) The original system scored a 20 song catalog against a hardcoded user profile using a six-feature weighted sum (mood, energy, valence, acousticness, danceability, tempo, genre). It demonstrated the fundamentals of feature similarity scoring and behavioral signals (liked/skipped history).

---


# Music Recommender Simulation (VibeMatch AI)

## Project Summary
This project is a content-based music recommender built in Python. It scores every song in a catalog against a user's taste profile and returns the top matches with human-readable explanations. The system models user preference across seven audio dimensions — energy, valence, acousticness, danceability, tempo, mood, and genre — and combines them into a single relevance score using a weighted sum. Behavioral history (songs previously liked or skipped) is layered on top as a final score adjustment, mirroring the signal prioritization used by platforms like Spotify and SoundCloud.
...
The recommender runs each song through a four-step scoring pipeline and returns the top N matches ranked by relevance score.

### The Two Core Objects

**Song** — stores the raw features loaded from `data/songs.csv` or `data/kaggle_tracks.csv`

| Field         | Type      | Role                                 |
|-------------- |---------- |--------------------------------------|
| id            | int       | Unique identifier                    |
| title         | string    | Display only, not scored             |
| artist        | string    | Display only, used for diversity     |
| genre         | string    | Categorical — scored as match/no-match |
| mood          | string    | Categorical — scored as match/no-match |
| energy        | float 0–1 | Continuous — scored by distance      |
| valence       | float 0–1 | Continuous — scored by distance      |
| danceability  | float 0–1 | Continuous — scored by distance      |
| acousticness  | float 0–1 | Continuous — scored by distance      |
...
| preferred_tempo_bpm    | float       | Numeric target (normalized) |
...
1. **Normalize Tempo**
     - `tempo_bpm` is normalized to 0–1 scale before scoring.
2. **Per-Feature Similarity**
     - Continuous features: `similarity = 1 - |song_value - user_preference|`
     - Categorical features: 1.0 if match, 0.0 if not
3. **Weighted Sum**
...
4. **Behavioral Adjustment**
     - liked_ids: ×1.2 (capped at 1.0)
     - skipped_ids: ×0.5
     - conflict: ×0.5, surfaced in explanation
...
### Prerequisites
- Python 3.10+
- Kaggle Spotify Tracks dataset (download and save as `data/dataset.csv`)
- (Optional) Google Gemini API key for agent mode
...
### 3. Prepare the data catalog (run once)
```bash

### Semantic Retrieval (RAG)

### 4. Build the embedding index (run once)
```bash
...
### 6. Run the original rule-based recommender
```bash
- valence (0.23)
- acousticness (0.22)
...
- Download the [Kaggle Spotify Tracks dataset](https://www.kaggle.com/datasets/)
- Save as `data/dataset.csv`
- Run `scripts/prepare_data.py` to clean and format the data (produces `data/kaggle_tracks.csv`)
- Run `scripts/build_index.py` to generate the semantic embedding index (produces `data/embeddings.npy`)
- These scripts must be run before using the recommender
...
- **Why keep the original scoring engine unchanged?** The weighted feature sum from Module 1 is a well-calibrated reranker. Its weights (energy 25%, valence 20%) reflect Spotify's own published research on which audio features best predict satisfaction. Adding semantic search on top improves candidate quality without replacing logic that was already working.
- **Why sentence-transformers instead of a Gemini embedding call?** all-MiniLM-L6-v2 runs locally, costs nothing, and produces high-quality semantic embeddings for music descriptions. Using Gemini embeddings would add API cost and latency to every query. Local embeddings are also reproducible — the same query always returns the same candidates.
- **Why few-shot prompting for profile extraction?** The profile extraction prompt includes four worked examples of query → JSON profile mappings. This "fine-tunes" Gemini's behavior for the music domain without any model training. Without the examples, Gemini produces inconsistent JSON structures. With them, field names and value ranges are stable across diverse queries.
...
- **Why retry on low confidence instead of just returning results?** The output guardrail catches cases where the semantic search retrieved musically adjacent but mismatched songs. The retry appends the guardrail's flag message to the query (e.g., "more hip-hop less rock") before re-searching, giving the embedding model better signal. In testing, one retry is usually sufficient to cross the 0.6 confidence threshold.
